...
\newcommand{\Overlap}[5]{%
  ...
  % NOTE: pgfmath macros must be valid TeX control sequences (letters only).
  \pgfmathsetmacro{\xZero}{#1 + #3*\slotW}%
  \pgfmathsetmacro{\xOne}{#1 + #4*\slotW}%
  \path[overlapFill] (\xZero,#2) rectangle (\xOne,#2+#5);
}
